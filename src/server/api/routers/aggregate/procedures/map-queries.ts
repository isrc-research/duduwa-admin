...
export const getMapEntities = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    conditions.push(sql`
      ${duduwaAggregateBuilding.building_gps_latitude}::numeric BETWEEN ${south.toString()}::numeric AND ${north.toString()}::numeric AND
      ${duduwaAggregateBuilding.building_gps_longitude}::numeric BETWEEN ${west.toString()}::numeric AND ${east.toString()}::numeric
    `);

    // Apply optional filters
    if (wardId) {
      conditions.push(
        eq(duduwaAggregateBuilding.ward_number, parseInt(wardId)),
      );
    }

    if (areaCode) {
      conditions.push(
        eq(duduwaAggregateBuilding.area_code, parseInt(areaCode)),
      );
    }

    if (enumeratorId) {
      conditions.push(
        eq(duduwaAggregateBuilding.enumerator_id, enumeratorId),
      );
    }

    if (mapStatus) {
      conditions.push(eq(duduwaAggregateBuilding.map_status, mapStatus));
    }
    ...
    const buildingEntities = await ctx.db
      .select({
        ...
        accuracy: duduwaAggregateBuilding.building_gps_accuracy,
      ...
      .orderBy(duduwaAggregateBuilding.created_at)
      .limit(limit);
...
export const getMapEntityById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (type === "building") {
      ...
      const jsonbField =
        type === "household"
          ? duduwaAggregateBuilding.households
          : duduwaAggregateBuilding.businesses;
      const conditionField = type === "household" ? "households" : "businesses";

      const buildingQuery = await ctx.db
        .select({
          building: duduwaAggregateBuilding,
        })
        .from(duduwaAggregateBuilding)
        .where(
...
export const getClusterPoints = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    const buildingEntities = await ctx.db
      ...
      .where(
        sql`
          ${duduwaAggregateBuilding.building_gps_latitude}::numeric BETWEEN ${latMin.toString()}::numeric AND ${latMax.toString()}::numeric AND
          ${duduwaAggregateBuilding.building_gps_longitude}::numeric BETWEEN ${lngMin.toString()}::numeric AND ${lngMax.toString()}::numeric
        `,
      )
      .limit(limit);
