  ...
  .query(async ({ ctx, input }) => {
    ...
    const [
      ...
      ctx.db
        ...
        .where(eq(duduwaAgriculturalLand.familyId, input.id)),
      ctx.db
        .select()
        .from(duduwaAnimal)
        .where(eq(duduwaAnimal.familyId, input.id)),
      ctx.db
        .select()
        .from(duduwaAnimalProduct)
        .where(eq(duduwaAnimalProduct.familyId, input.id)),
      ctx.db
        .select()
        .from(duduwaCrop)
        .where(eq(duduwaCrop.familyId, input.id)),
      ctx.db
        .select()
        .from(duduwaIndividual)
        .where(eq(duduwaIndividual.familyId, input.id)),
    ]);
